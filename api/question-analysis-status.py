"""GET /api/question-analysis-status?courseId=... — Check question analysis status.

If completed analysis exists in KV, return it.
...
def _fetch_batch_results(batch_id: str) -> list[dict]:
    """Fetch all results from a completed batch. Returns list of parsed results."""
    results = []
    try:
        resp = requests.get(
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 1. Check for completed analysis
        saved = kv_get(f"question_analysis:{course_id}")
        ...
        job = kv_get(f"question_analysis_job:{course_id}")
        ...
        if processing_status == "ended":
            ...
            if request_counts.get("succeeded", 0) > 0:
                ...
                if chunk_results:
                    # Merge all chunks into a single analysis object
                    merged = {}
                    ...
                    kv_set(f"question_analysis:{course_id}", analysis_data)
                    kv_delete(f"question_analysis_job:{course_id}")

"""GET /api/explanation-status?courseId=... — Check explanation generation status.

If completed explanations exist in KV, return them.
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        job = kv_get(f"explanation_job:{course_id}")
        ...
        if processing_status == "ended":
            ...
            if request_counts.get("succeeded", 0) > 0:
                ...
                if chunk_results:
                    # Merge all chunks into a single explanations object
                    merged = {}
                    ...
                    kv_set(f"explanations:{course_id}", explanation_data)
                    kv_delete(f"explanation_job:{course_id}")

                    send_json(self, {
                        "status": "done",
                        "explanations": merged,
                        "questionCount": len(merged),
                        "generatedAt": explanation_data["generatedAt"],
                        "model": explanation_data["model"],
                    })
                    return
                else:
                    kv_delete(f"explanation_job:{course_id}")

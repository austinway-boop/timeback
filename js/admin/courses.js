    /* ====================================================================
       Courses Page – Full Course Management with Caching
       ==================================================================== */
    ...
    function showSkeletonCards(count) {
        const colors = ['card-theme-teal', 'card-theme-purple', 'card-theme-pink', 'card-theme-orange'];
        document.getElementById('courses-grid').innerHTML = Array.from({ length: count }, (_, i) => `
            <div class="skeleton-course-card">
                <div class="skeleton-course-image skeleton ${colors[i % 4]}" style="height:100px;opacity:0.3;"></div>
                <div class="skeleton-course-body">
                    <div class="skeleton skeleton-text lg" style="width:70%;margin-bottom:10px;"></div>
                    <div class="skeleton skeleton-text" style="width:50%;margin-bottom:8px;"></div>
                    <div class="skeleton skeleton-text sm" style="width:35%;margin-bottom:10px;"></div>
                    <div class="skeleton skeleton-text xs" style="width:25%;"></div>
                </div>
            </div>`).join('');
    }
    ...
    document.addEventListener('DOMContentLoaded', async function () {
        ...
        const cachedCourses = getCache('al_courses');
        const cachedClasses = getCache('al_classes');

        if (cachedCourses && cachedClasses) {
    ...
    async function refreshData(silent) {
        try {
            ...
            setCache('al_courses', allCourses);
            setCache('al_classes', allClasses);

            updateStats();
            if (!silent) populateGradeFilter();
            filterAndRender();
        } catch (e) {
            if (!silent) {
    ...
    function updateStats() {
        ...
        const subjectMap = {};
        allCourses.forEach(c => (c.subjects || []).forEach(s => { subjectMap[s] = (subjectMap[s] || 0) + 1; }));
    ...
    function populateGradeFilter() {
        ...
        // Clear existing options except the first "All Grades"
    ...
    function filterAndRender() {
        ...
        filteredCourses = allCourses.filter(c => {
            ...
            return (!query || title.includes(query) || code.includes(query) || subjects.includes(query))
                && (!statusFilter || c.status === statusFilter)
                && (!gradeFilter  || (c.grades || []).includes(gradeFilter));
        });
    ...
    async function viewCourseDetail(courseId) {
        ...
        if (!course) { body.innerHTML = '<p class="error-text">Course not found.</p>'; return; }
        ...
        const lastMod  = course.dateLastModified ? new Date(course.dateLastModified).toLocaleDateString() : '\u2014';

        function renderModal(lineItems, loading) {
            body.innerHTML = `
                ...
                <div class="detail-tabs">
                    ...
                    <button class="detail-tab" onclick="switchDetailTab(this,'lineitems-tab')">Line Items${loading ? '' : ' (' + lineItems.length + ')'}</button>
                </div>
                <div id="classes-tab">
                    ${courseClasses.length ? `<table class="detail-table"><thead><tr><th>Title</th><th>Code</th><th>Type</th><th>Status</th></tr></thead><tbody>${courseClasses.map(cl => `<tr><td>${esc(cl.title)||'\u2014'}</td><td>${esc(cl.classCode)||'\u2014'}</td><td>${esc(cl.classType)||'\u2014'}</td><td><span class="status-badge status-${esc(cl.status)}">${esc(cl.status)||'\u2014'}</span></td></tr>`).join('')}</tbody></table>` : '<div class="detail-empty">No classes associated with this course.</div>'}
                </div>
                <div id="lineitems-tab" style="display:none;">
                    ${loading ? '<div style="padding:8px 0;"><div class="skeleton" style="height:34px;border-radius:6px;margin-bottom:8px;"></div><div class="skeleton" style="height:38px;border-radius:6px;margin-bottom:4px;"></div><div class="skeleton" style="height:38px;border-radius:6px;"></div></div>' :
                    (lineItems.length ? `<table class="detail-table"><thead><tr><th>Title</th><th>Due Date</th><th>Max Score</th><th>Status</th></tr></thead><tbody>${lineItems.map(li => `<tr><td>${esc(li.title)||'\u2014'}</td><td>${li.dueDate ? new Date(li.dueDate).toLocaleDateString() : '\u2014'}</td><td>${li.resultValueMax != null ? esc(li.resultValueMax) : '\u2014'}</td><td><span class="status-badge status-${esc(li.status)}">${esc(li.status)||'\u2014'}</span></td></tr>`).join('')}</tbody></table>` : '<div class="detail-empty">No line items found.</div>')}
                </div>`;
        }
    ...
    /* ====================================================================
       ...
       ==================================================================== */
    let assignCourseId = null, assignSelectedUser = null;
    ...
    function filterAssignList() {
        ...
        if (!filtered.length) { list.innerHTML = '<div class="detail-empty">No students found.</div>'; return; }
        list.innerHTML = filtered.slice(0, 50).map(u => {
            const name = `${u.givenName || ''} ${u.familyName || ''}`.trim();
            const sel = assignSelectedUser && assignSelectedUser.sourcedId === u.sourcedId;
            return `<div class="student-list-item ${sel ? 'selected' : ''}" onclick="selectStudent('${esc(u.sourcedId)}')"><div class="user-cell-avatar">${esc((u.givenName||'?')[0].toUpperCase())}</div><div><div class="user-cell-name">${esc(name)}</div><div class="user-cell-username">${esc(u.email||u.username||'')}</div></div></div>`;
        }).join('');
    }
    ...
    function selectStudent(userId) {
        ...
        filterAssignList();
        const el = document.getElementById('assign-confirm');
        if (assignSelectedUser) {
            const course = allCourses.find(c => c.sourcedId === assignCourseId);
            el.style.display = '';
            document.getElementById('assign-btn').textContent = `Assign "${course ? course.title : 'course'}" to ${assignSelectedUser.givenName} ${assignSelectedUser.familyName}`;
        } else { el.style.display = 'none'; }
    }

    function confirmAssign() {
        const course = allCourses.find(c => c.sourcedId === assignCourseId);
        const sn = esc(`${assignSelectedUser.givenName||''} ${assignSelectedUser.familyName||''}`.trim());
        const cn = esc(course ? course.title : 'this course');
        document.getElementById('assign-modal-body').innerHTML = `<div class="assign-success"><i class="fa-solid fa-circle-check"></i><p><strong>${cn}</strong> assigned to <strong>${sn}</strong></p><p class="subtitle">Assignment requested (API integration pending).</p></div>`;
    }
